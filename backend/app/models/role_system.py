...
class EnhancedRole(Base, TimestampedMixin):
    ...
    # =============================================================================
    # Business Logic Methods - Permission Management
    # =============================================================================
    ...
    def get_permissions(self) -> Set[Permission]:
        ...
        # Базовые разрешения по типу роли
        if self.system_role:
            permissions.update(self._get_system_role_permissions(self.system_role))
        if self.company_role:
            permissions.update(self._get_company_role_permissions(self.company_role))
        if self.department_role:
            permissions.update(
                self._get_department_role_permissions(self.department_role)
            )
        if self.team_role:
            permissions.update(self._get_team_role_permissions(self.team_role))
        if self.project_role:
            permissions.update(self._get_project_role_permissions(self.project_role))

        # Дополнительные разрешения из конфигурации
        if self.permissions_config:
            custom_permissions = self.permissions_config.get(
    ...
    def can_assign_to_scope(self, scope: RoleScope) -> bool:
        """Может ли роль быть назначена на указанном уровне"""
        # Системные роли можно назначать только на системном уровне
        if self.is_system:
            return scope == RoleScope.SYSTEM

        # Роли компании можно назначать на уровне компании и ниже
        if self.scope == RoleScope.COMPANY:
            return scope in [
                RoleScope.COMPANY,
        ...
        # Роли департамента - на уровне департамента и ниже
        if self.scope == RoleScope.DEPARTMENT:
            return scope in [RoleScope.DEPARTMENT, RoleScope.TEAM, RoleScope.PROJECT]

        # Роли команды - только на уровне команды и проекта
        if self.scope == RoleScope.TEAM:
            return scope in [RoleScope.TEAM, RoleScope.PROJECT]

        # Роли проекта - только на уровне проекта
        if self.scope == RoleScope.PROJECT:
            return scope == RoleScope.PROJECT

        return False

    # =============================================================================
    # Permission Sets by Role Type
    # =============================================================================

    def _get_system_role_permissions(self, role: SystemRole) -> Set[Permission]:
        """Получить разрешения для системной роли"""
        if role == SystemRole.SYSTEM_ADMIN:
            # Все разрешения
            return set(Permission)
        elif role == SystemRole.PLATFORM_ADMIN:
            return {
        ...
        elif role == SystemRole.SUPPORT_ADMIN:
            return {
                Permission.VIEW_SYSTEM_LOGS,
        ...
        elif role == SystemRole.BILLING_ADMIN:
            return {
                Permission.MANAGE_GLOBAL_BILLING,
                Permission.MANAGE_COMPANY_BILLING,
                Permission.VIEW_COMPANY_BILLING,
                Permission.MANAGE_COMPANY_SUBSCRIPTION,
            }
        # Add more system roles...
        return set()

    def _get_company_role_permissions(self, role: CompanyRole) -> Set[Permission]:
        """Получить разрешения для роли в компании"""
        ...
        # Add more company roles...
        return set()

    def _get_department_role_permissions(self, role: DepartmentRole) -> Set[Permission]:
        """Получить разрешения для роли в департаменте"""
        if role == DepartmentRole.DEPARTMENT_HEAD:
            return {
                Permission.MANAGE_DEPARTMENT,
        ...
        elif role == DepartmentRole.MANAGER:
            return {
        ...
        # Add more department roles...
        return set()

    def _get_team_role_permissions(self, role: TeamRole) -> Set[Permission]:
        """Получить разрешения для роли в команде"""
        ...
        elif role == TeamRole.SCRUM_MASTER:
            return {
    ...
    def _get_project_role_permissions(self, role: ProjectRole) -> Set[Permission]:
        """Получить разрешения для роли в проекте"""
        ...
        elif role == ProjectRole.DEVELOPER:
            return {
                Permission.VIEW_PROJECT,
                Permission.CREATE_REQUIREMENT,
        ...
        elif role == ProjectRole.QA_ENGINEER:
            return {
                Permission.VIEW_PROJECT,
                ...
                Permission.MANAGE_TEST_PLANS,
                Permission.APPROVE_TEST_RESULTS,
            }
...
class UserRoleAssignment(Base, TimestampedMixin):
    ...
    def get_permissions(self) -> Set[Permission]:
        """Получить разрешения из назначенной роли"""
    ...
    def can_access_resource(self, resource_type: str, resource_id: int) -> bool:
        """Проверить, может ли назначение предоставить доступ к ресурсу"""
        if not self.is_valid:
            return False

        # Логика проверки доступа в зависимости от типа ресурса и контекста
