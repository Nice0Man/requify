...
class TestCaseNotFoundError(TestCaseServiceError):
    ...
    def __init__(self, test_case_id: int):
        super().__init__(f"Test case with id {test_case_id} not found", "TEST_CASE_NOT_FOUND")
...
class ITestCaseRepository(ABC):
    ...
    @abstractmethod
    async def get_by_id(self, db: AsyncSession, test_case_id: int) -> Optional[TestCase]:
        pass
    
    @abstractmethod
    async def create(self, db: AsyncSession, test_case_data: TestCaseCreate) -> TestCase:
        pass
...
class ITestCaseValidator(ABC):
    ...
    @abstractmethod
    async def validate_create(self, db: AsyncSession, test_case_data: TestCaseCreate) -> None:
        pass
...
class TestCaseBusinessValidator(ITestCaseValidator):
    ...
    async def validate_create(self, db: AsyncSession, test_case_data: TestCaseCreate) -> None:
        if not test_case_data.name or len(test_case_data.name.strip()) < 3:
            raise TestCaseValidationError("Test case name must be at least 3 characters long")
...
class TestCaseManagementService(BaseService):
    ...
    async def create_test_case(
        ...
        try:
            self._log_operation("create_test_case", {
                "test_case_name": test_case_data.name,
                "current_user_id": current_user.id
            })
            
            # Проверка прав на создание
            if not await self._permission_checker.can_create_test_case(db, current_user):
    ...
    async def get_test_case(
        ...
        try:
            self._log_operation("get_test_case", {
    ...
    async def update_test_case(
        ...
        try:
            self._log_operation("update_test_case", {
                "test_case_id": test_case_id,
                "current_user_id": current_user.id
            })
            ...
            # Обновление тестового случая
            updated_test_case = await self._repository.update(db, test_case, update_data)
    ...
    async def delete_test_case(
        ...
        try:
            self._log_operation("delete_test_case", {
                "test_case_id": test_case_id,
                "current_user_id": current_user.id
            })
