...
class ServiceIntegrationTester:
        ...
        try:
            ...
            if len(registered_services) > 0:
                ...
                self.log_test_result(
                    "service_factory", 
                    False, 
                    "No services registered in factory"
                )
        ...
        try:
            ...
            if test_event_data == test_data:
                self.log_test_result(
                    "event_dispatcher", 
                    True, 
                    "Event dispatcher works correctly"
                )
            else:
                self.log_test_result(
                    "event_dispatcher", 
    ...
    def test_service_configuration(self):
        ...
        try:
            ...
            default_value = service_config.get_setting("test_service", "nonexistent", "default")
        ...
        try:
            ...
            if hasattr(auth_service, '_handle_error'):
                self.log_test_result(
                    "error_handling", 
                    True, 
                    "Services have error handling capabilities"
                )
            else:
                self.log_test_result(
                    "error_handling", 
                    False, 
                    "Services missing error handling methods"
                )
        ...
        try:
            ...
            if hasattr(auth_service, '_log_operation'):
                ...
                self.log_test_result(
                    ...
                    True, 
                    "Services have logging capabilities"
                )
            else:
                self.log_test_result(
                    "logging_functionality", 
                    False, 
                    "Services missing logging methods"
                )
    ...
    async def run_all_tests(self):
        ...
        if self.failed_tests == 0:
            logger.info("🎉 All integration tests passed! The refactored services are working correctly.")
        else:
            logger.warning(f"⚠️  {self.failed_tests} tests failed. Check the logs for details.")
