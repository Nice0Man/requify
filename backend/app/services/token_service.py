...
class JWTTokenGenerator(ITokenGenerator):
    ...
    async def generate_token_pair(
        ...
        access_expires = timedelta(minutes=settings.security.access_token_expire_minutes)
        ...
        access_token_info = TokenInfo(
            token=access_token,
            token_type=TokenType.ACCESS,
            expires_at=datetime.now(UTC) + access_expires,
            scope=scopes,
            user_id=user.id
        )
...
class TokenService(BaseService):
    ...
    async def create_tokens_for_user(
        ...
        try:
            self._log_operation("create_tokens_for_user", {
                "user_id": user.id,
            ...
            if not self._security_manager.check_token_security(user, ip_address, user_agent):
            ...
            # Генерация токенов
            token_pair = await self._generator.generate_token_pair(user, scopes, remember_me)
            ...
            refresh_token_id = await self._storage.store_refresh_token(
                db, user.id, token_data, token_pair.refresh_token.expires_at
            )
    ...
    async def validate_access_token(
        ...
        try:
            self._log_operation("validate_access_token", {"token_provided": bool(token)})
    ...
    async def refresh_access_token(
        ...
        try:
            self._log_operation("refresh_access_token", {"token_provided": bool(refresh_token)})
            
            # Валидация refresh токена
            user, _ = await self._validator.validate_token(refresh_token, TokenType.REFRESH, db)
    ...
    async def revoke_refresh_token(
        ...
        """Отозвать refresh токен."""
        try:
            self._log_operation("revoke_refresh_token", {
                "token_id": token_id,
                "user_id": user.id
            })
    ...
    async def revoke_all_user_tokens(
        ...
        try:
            self._log_operation("revoke_all_user_tokens", {
                "user_id": user.id,
