...
class SimpleEmailTemplate(IEmailTemplate):
    ...
    def __init__(self, subject_template: str, body_template: str, html_template: Optional[str] = None):
...
class EmailService(BaseService):
        ...
        try:
            if hasattr(settings, 'email'):
                ...
                config = {
                    ...
                    'from_email': getattr(email_config, 'from_email', 'noreply@example.com'),
                    'from_name': getattr(email_config, 'from_name', 'System'),
                }
                
                development_mode = getattr(settings.run, 'env', 'development') == 'development'
                smtp_configured = config['smtp_host'] and config['smtp_host'] not in ('', 'localhost')
                ...
                if development_mode and not smtp_configured:
                    ...
                    logger.info(f"Using SMTP email backend: {config['smtp_host']}:{config['smtp_port']}")
            else:
                self._backend = ConsoleEmailBackend()
    ...
    async def send_templated_email(
        ...
        try:
            self._log_operation("send_templated_email", {
    ...
    async def send_email_verification(
        ...
        try:
            verification_url = f"https://example.com/verify-email?token={verification_token}"
