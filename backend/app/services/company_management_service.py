...
class ICompanyValidator(ABC):
    ...
    @abstractmethod
    async def validate_create(self, db: AsyncSession, company_data: CompanyCreate) -> None:
...
class CompanyRepository(ICompanyRepository):
    ...
    async def delete(self, db: AsyncSession, company_id: int) -> bool:
        ...
        await self.crud.update(
            db, 
            db_obj=company, 
            obj_in={"status": CompanyStatus.DELETED}
        )
        return True
...
class CompanyBusinessValidator(ICompanyValidator):
    ...
    async def validate_create(self, db: AsyncSession, company_data: CompanyCreate) -> None:
        ...
        if existing_company:
            raise CompanyValidationError(f"Company with name '{company_data.name}' already exists")
        ...
        if len(company_data.name) < 2:
            raise CompanyValidationError("Company name must be at least 2 characters long")
    ...
    async def validate_update(
        ...
        if update_data.name:
            ...
            if existing_company and existing_company.id != company.id:
                raise CompanyValidationError(f"Company with name '{update_data.name}' already exists")
...
class CompanyManagementService(BaseService):
    ...
    async def get_companies_list(
        ...
        """
        try:
            self._log_operation("get_companies_list", {
                "skip": skip,
                "limit": limit,
                "search": search,
                "status": status,
    ...
    async def get_company_by_id(
        ...
        """
        try:
            self._log_operation("get_company_by_id", {
    ...
    async def create_company(
        ...
        """
        try:
            self._log_operation("create_company", {
    ...
    async def update_company(
        ...
        """
        try:
            self._log_operation("update_company", {
                "company_id": company_id,
                "current_user_id": current_user.id
            })
            
            # Проверка прав доступа
            if not await self._permission_checker.can_manage_company(db, current_user, company_id):
    ...
    async def delete_company(
        ...
        """
        try:
            self._log_operation("delete_company", {
                "company_id": company_id,
                "current_user_id": current_user.id
            })
    ...
    async def get_company_statistics(
        ...
        """
        try:
            self._log_operation("get_company_statistics", {
                "company_id": company_id,
                "current_user_id": current_user.id
            })
            ...
            # Проверка прав доступа
            if not await self._permission_checker.can_view_companies(db, current_user):
                raise PermissionError("Insufficient permissions to view company statistics")
