...
class NotificationService(BaseService):
    ...
    def register_channel(self, channel: INotificationChannel):
        ...
        self._log_operation("register_channel", {"channel": channel.get_channel_type().value})
    
    async def send_notification(self, context: NotificationContext) -> List[NotificationResult]:
        """Отправить уведомление."""
        try:
            self._log_operation("send_notification", {
            ...
            if not template:
                raise NotificationServiceError(f"Template not found for type: {context.type}")
    ...
    async def send_requirement_notification(
        ...
        try:
            data = {
                ...
                "project_name": requirement.project.name if requirement.project else "Unknown",
                **(additional_data or {})
            }
