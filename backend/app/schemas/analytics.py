...
class AnalyticsBase(BaseSchema):
    ...
    is_active: bool = Field(
        True,
        description=StandardDescriptions.IS_ACTIVE
    )
...
class MetricValue(BaseSchema):
    ...
    metric_metadata: Optional[Dict[str, Any]] = Field(None, description="Дополнительные данные", alias="metadata")
...
class DashboardMetric(BaseSchema):
    ...
    title: str = Field(..., max_length=FieldLimits.MEDIUM_STRING_MAX, description="Заголовок")
    value: Union[int, float] = Field(..., description="Текущее значение")
    previous_value: Optional[Union[int, float]] = Field(None, description="Предыдущее значение")
    ...
    priority: PriorityEnum = Field(PriorityEnum.MEDIUM, description="Приоритет отображения")
...
class ChartData(BaseSchema):
    ...
    datasets: List[Dict[str, Any]] = Field(default_factory=list, description="Наборы данных")
    chart_type: ChartTypeEnum = Field(ChartTypeEnum.LINE, description="Тип графика")
...
class MetricValueUpdate(BaseSchema):
    ...
    @field_validator("value")
    @classmethod
    def validate_value(cls, v: Optional[Union[int, float]]) -> Optional[Union[int, float]]:
        """Валидация значения"""
...
class MetricValueFilter(DateRangeFilter):
    ...
    min_value: Optional[Union[int, float]] = Field(None, description="Минимальное значение")
    max_value: Optional[Union[int, float]] = Field(None, description="Максимальное значение")
...
class DashboardStatsRequest(BaseSchema):
    ...
    metric_types: Optional[List[MetricTypeEnum]] = Field(None, description="Типы метрик")
...
class AnalyticsSummary(BaseSchema):
    ...
    by_period: List[CountStatistic] = Field(default_factory=list, description="По периодам")
