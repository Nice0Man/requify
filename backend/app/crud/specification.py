...
from app.schemas.specification import SpecificationCreate, SpecificationUpdate, SpecificationStatus


class CRUDSpecification(CRUDBase[Specification, SpecificationCreate, SpecificationUpdate]):
    """CRUD операции для спецификаций"""
    ...
    async def add_requirements_to_specification(
        ...
        *,
        ...
        valid_requirement_ids = [req_id for req_id in requirement_ids if req_id in existing_ids]
        ...
        new_requirement_ids = [req_id for req_id in valid_requirement_ids if req_id not in already_linked]
    ...
    async def reorder_requirements_in_specification(
        ...
        *,
        ...
        for i, req_id in enumerate(requirement_order):
            stmt = update(specification_requirements).where(
                and_(
                    specification_requirements.c.specification_id == specification_id,
