...
from app.services.specification_management_service import specification_management_service
from .schemas import (
...
async def get_specification_requirements(
    ...
    """
    try:
        requirements = await specification_management_service.get_specification_requirements(
            db=db,
            spec_id=spec_id,
            skip=skip,
            limit=limit,
...
async def download_specification(
    ...
    """
    try:
        download_info = await specification_management_service.prepare_specification_download(
            db=db,
            spec_id=spec_id,
            format=format,
            current_user=current_user,
...
async def get_specification_coverage(
    ...
    """
    try:
        coverage = await specification_management_service.analyze_specification_coverage(
            db=db, spec_id=spec_id, current_user=current_user
        )
