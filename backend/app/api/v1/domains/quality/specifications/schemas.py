...
class SpecificationCreateRequest(BaseSchema):
    ...
    title: str = Field(..., min_length=1, max_length=255, description="Заголовок спецификации")
    description: Optional[str] = Field(None, max_length=1000, description="Описание спецификации")
    project_id: int = Field(..., description="ID проекта")
    content: str = Field(..., description="Содержимое спецификации")
    specification_type: SpecificationType = Field(SpecificationType.FUNCTIONAL, description="Тип спецификации")
...
class SpecificationUpdateRequest(BaseSchema):
    ...
    title: Optional[str] = Field(None, min_length=1, max_length=255, description="Заголовок")
...
class SpecificationListResponse(BaseSchema):
    ...
    specifications: List[SpecificationResponse] = Field(..., description="Список спецификаций")
...
class CoverageAnalysisResponse(BaseSchema):
    ...
    requirements_coverage: List[RequirementCoverage] = Field(..., description="Детали покрытия")
...
class SpecificationExportRequest(BaseSchema):
    ...
    specification_ids: List[int] = Field(..., description="IDs спецификаций для экспорта")
...
class ImportResultResponse(BaseSchema):
    ...
    imported_specifications: List[int] = Field([], description="IDs импортированных спецификаций")
