...
class QualityMetricsResponse(BaseSchema):
    ...
    test_execution: TestExecutionMetrics = Field(..., description="Метрики выполнения тестов")
...
class CoverageItem(BaseSchema):
    ...
    last_tested: Optional[datetime] = Field(None, description="Дата последнего тестирования")
...
class DefectAnalysisResponse(BaseSchema):
    ...
    total_defects: int = Field(..., description="Общее количество дефектов")
    defects_by_severity: List[DefectCategory] = Field(..., description="Дефекты по серьезности")
    defects_by_category: List[DefectCategory] = Field(..., description="Дефекты по категориям")
    defects_by_component: List[DefectCategory] = Field(..., description="Дефекты по компонентам")
...
class AutomationMetrics(BaseSchema):
    ...
    automated_execution_time: float = Field(..., description="Время автоматизированного выполнения")
...
class AutomationReportResponse(BaseSchema):
    ...
    project_id: Optional[int] = Field(None, description="ID проекта")
    period: str = Field(..., description="Период отчета")
    automation_metrics: AutomationMetrics = Field(..., description="Метрики автоматизации")
    automation_trends: List[Dict[str, Any]] = Field([], description="Тренды автоматизации")
