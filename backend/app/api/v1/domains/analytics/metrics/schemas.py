...
class MetricDefinitionRequest(BaseSchema):
    ...
    description: Optional[str] = Field(None, max_length=1000, description="Описание метрики")
...
class MetricValueRequest(BaseSchema):
    ...
    metric_metadata: Dict[str, Any] = Field({}, description="Дополнительные метаданные", alias="metadata")
...
class MetricValueResponse(BaseSchema):
    ...
    metric_metadata: Dict[str, Any] = Field(..., description="Метаданные", alias="metadata")
...
class AggregatedMetric(BaseSchema):
    ...
    metric_id: int = Field(..., description="ID метрики")
    metric_name: str = Field(..., description="Название метрики")
    aggregated_value: Union[float, int] = Field(..., description="Агрегированное значение")
    aggregation_type: AggregationType = Field(..., description="Тип агрегации")
...
class MetricAggregationResponse(BaseSchema):
    ...
    aggregated_metrics: List[AggregatedMetric] = Field(..., description="Агрегированные метрики")
...
class CustomMetricResponse(BaseSchema):
    ...
    last_calculated: Optional[datetime] = Field(None, description="Время последнего расчета")
    current_value: Optional[Union[float, int]] = Field(None, description="Текущее значение")
    created_by: int = Field(..., description="ID создателя")
    created_at: datetime = Field(..., description="Дата создания")
...
class MetricThreshold(BaseSchema):
    ...
    metric_id: int = Field(..., description="ID метрики")
    warning_threshold: Optional[Union[float, int]] = Field(None, description="Порог предупреждения")
    critical_threshold: Optional[Union[float, int]] = Field(None, description="Критический порог")
    target_value: Optional[Union[float, int]] = Field(None, description="Целевое значение")
    acceptable_range_min: Optional[Union[float, int]] = Field(None, description="Минимум допустимого диапазона")
    acceptable_range_max: Optional[Union[float, int]] = Field(None, description="Максимум допустимого диапазона")
...
class MetricCorrelationAnalysis(BaseSchema):
    ...
    statistical_significance: float = Field(..., description="Статистическая значимость")
...
class MetricAnalysisResponse(BaseSchema):
    ...
    trend_analyses: List[MetricTrendAnalysis] = Field(..., description="Анализы трендов")
    correlation_analyses: List[MetricCorrelationAnalysis] = Field(..., description="Анализы корреляций")
...
class RealtimeMetricUpdate(BaseSchema):
    ...
    previous_value: Optional[Union[float, int]] = Field(None, description="Предыдущее значение")
    change_percentage: Optional[float] = Field(None, description="Изменение в процентах")
...
class RealtimeMetricsResponse(BaseSchema):
    ...
    metrics_updates: List[RealtimeMetricUpdate] = Field(..., description="Обновления метрик")
