...
class AnalyticsReportRequest(BaseSchema):
    ...
    report_type: AnalyticsReportType = Field(..., description="Тип аналитического отчета")
...
class ReportTemplateResponse(BaseSchema):
    ...
    report_type: AnalyticsReportType = Field(..., description="Тип отчета")
    parameters: List[ReportTemplateParameter] = Field(..., description="Параметры шаблона")
...
class PerformanceAnalysisResponse(BaseSchema):
    ...
    report_id: str = Field(..., description="ID отчета")
    analysis_period: str = Field(..., description="Период анализа")
    performance_metrics: List[PerformanceMetric] = Field(..., description="Метрики производительности")
    overall_performance_score: float = Field(..., description="Общий балл производительности")
    improvement_areas: List[str] = Field(..., description="Области для улучшения")
    optimization_recommendations: List[str] = Field(..., description="Рекомендации по оптимизации")
    generated_at: datetime = Field(..., description="Дата генерации")
...
class CustomAnalyticsResponse(BaseSchema):
    ...
    analytics_metadata: Dict[str, Any] = Field(..., description="Метаданные", alias="metadata")
    generated_at: datetime = Field(..., description="Дата генерации")
