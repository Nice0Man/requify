...
class AnalyticsDependencies:
    ...
    def time_range_params() -> Callable:
        ...
        def validate_time_range(
            time_range: TimeRange = Query(TimeRange.LAST_30_DAYS, description="Predefined time range"),
            start_date: Optional[date] = Query(None, description="Custom start date (for custom range)"),
            end_date: Optional[date] = Query(None, description="Custom end date (for custom range)"),
        ) -> Dict[str, datetime]:
    ...
    def aggregation_params() -> Callable:
        ...
        def validate_aggregation(
            ...
            metric_type: MetricType = Query(MetricType.COUNT, description="Aggregation type"),
            metric_field: Optional[str] = Query(None, description="Field to aggregate (for sum, avg, etc.)"),
