...
async def get_users(
    ...
    if search:
        users = await crud_user.search_users(
            db, search_term=search, skip=pagination["skip"], limit=pagination["limit"]
        )
    else:
...
@router.post(
    "/users/", response_model=UserDetailed, status_code=status.HTTP_201_CREATED
)
...
async def get_user_analytics(
    ...
    return {
        ...
        "metadata": (
            ...
            if export_params["include_metadata"]
            else None
        ),
    }
