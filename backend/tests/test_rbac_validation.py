...
def auth_headers():
    ...
    def _headers(user: User, scopes: list = None):
        ...
        token = create_access_token(subject=user.email, user_id=user.id, scopes=scopes)
...
class TestRBACPermissions:
    ...
    async def test_role_scope_mapping(self, test_users):
        ...
        for role_name, expected_scopes in EXPECTED_SCOPES_BY_ROLE.items():
            ...
            for scope in actual_scopes:
                assert (
                    scope in expected_scopes
                ), f"Role {role_name} has unexpected scope {scope}"

    @pytest.mark.asyncio
    async def test_admin_access_permissions(
        self, client: AsyncClient, test_users, auth_headers
    ):
        ...
        for role in admin_users:
            ...
            assert response.status_code in [
                200,
                404,
            ], f"Role {role} should have admin access"
        ...
        for role in non_admin_users:
            ...
            ), f"Role {role} should NOT have admin access"

    @pytest.mark.asyncio
    async def test_projects_permissions(
        self, client: AsyncClient, test_users, auth_headers
    ):
        ...
        for role_name, user in test_users.items():
            ...
            assert response.status_code in [
                200,
                404,
            ], f"Role {role_name} should have projects:read"
        ...
        for role in write_roles:
            ...
            assert response.status_code in [
                ...
                422,
                400,
            ], f"Role {role} should have projects:write"
        ...
        for role in non_delete_roles:
            ...
            assert (
                response.status_code == 401
            ), f"Role {role} should NOT have projects:delete"

    @pytest.mark.asyncio
    async def test_requirements_permissions(
        self, client: AsyncClient, test_users, auth_headers
    ):
        ...
        for role in write_roles:
            ...
            assert response.status_code in [
                ...
                422,
                400,
            ], f"Role {role} should have requirements:write"
        ...
        for role in no_write_roles:
            ...
            assert (
                response.status_code == 401
            ), f"Role {role} should NOT have requirements:write"

    @pytest.mark.asyncio
    async def test_releases_permissions(
        self, client: AsyncClient, test_users, auth_headers
    ):
        ...
        for role in write_roles:
            ...
            response = await client.post(
                "/api/v1/releases/",
                headers=headers,
                json={"name": "Test Release", "version": "1.0.0", "project_id": 1},
            )
            assert response.status_code in [
                201,
                422,
                400,
            ], f"Role {role} should have releases:write"
        ...
        for role in no_write_roles:
            ...
            assert (
                response.status_code == 401
            ), f"Role {role} should NOT have releases:write"

    @pytest.mark.asyncio
    async def test_testing_permissions(
        self, client: AsyncClient, test_users, auth_headers
    ):
        ...
        for role in execute_roles:
            ...
            response = await client.post(
                "/api/v1/testing/executions",
                headers=headers,
                json={"test_case_id": 1, "result": "passed"},
            )
            # Note: We expect 422/400 for missing data, not 401 for permissions
            assert (
                response.status_code != 401
            ), f"Role {role} should have testing:execute"
        ...
        for role in no_execute_roles:
            ...
            assert (
                response.status_code == 401
            ), f"Role {role} should NOT have testing:execute"

    @pytest.mark.asyncio
    async def test_dashboard_admin_features(
        self, client: AsyncClient, test_users, auth_headers
    ):
        ...
        for role in admin_roles:
            ...
            assert response.status_code in [
                200,
                404,
            ], f"Role {role} should access dashboard health"
            ...
            assert response.status_code in [
                200,
                404,
            ], f"Role {role} should access dashboard metrics"
        ...
        for role in non_admin_roles:
            ...
            assert (
                response.status_code == 401
            ), f"Role {role} should NOT access dashboard health"

            response = await client.get("/api/v1/dashboard/metrics", headers=headers)
            assert (
                response.status_code == 401
            ), f"Role {role} should NOT access dashboard metrics"
    ...
    async def test_scope_validation(
        ...
        response = await client.post(
            "/api/v1/projects/",
            headers=headers,
            json={"name": "Test Project", "code": "TEST001"},
        )
        assert (
            response.status_code == 401
        ), "Viewer should not have projects:write permission"

        # Viewer should not be able to access admin endpoints
        response = await client.get("/api/v1/admin/users", headers=headers)
        assert (
            response.status_code == 401
        ), "Viewer should not have admin:read permission"

    @pytest.mark.asyncio
    async def test_product_manager_permissions(
        self, client: AsyncClient, test_users, auth_headers
    ):
        ...
        response = await client.post(
            "/api/v1/projects/",
            headers=headers,
            json={"name": "PM Test Project", "code": "PMTEST001"},
        )
        ...
        response = await client.post(
            "/api/v1/releases/",
            headers=headers,
            json={"name": "PM Test Release", "version": "1.0.0", "project_id": 1},
        )
        assert response.status_code in [201, 422, 400], "PM should create releases"

    @pytest.mark.asyncio
    async def test_analyst_read_only_permissions(
        self, client: AsyncClient, test_users, auth_headers
    ):
        ...
        response = await client.post(
            "/api/v1/projects/",
            headers=headers,
            json={"name": "Analyst Test Project", "code": "ANALTEST001"},
        )
...
async def test_permission_hierarchy():
    ...
    product_manager = User(
        email="pm@test.com", role="product_manager", is_superuser=False
    )
    manager = User(email="manager@test.com", role="manager", is_superuser=False)
    senior_developer = User(
        email="senior_dev@test.com", role="senior_developer", is_superuser=False
    )
