...
class TestCRUDPerformance:
    ...
    def test_bulk_user_creation_performance(self, performance_session):
        ...
        assert creation_time < 2.0, f"Создание {user_count} пользователей заняло {creation_time:.2f} секунд"
    ...
    def test_bulk_user_query_performance(self, performance_session):
        ...
        assert query_time < 0.5, f"Запрос {user_count} пользователей занял {query_time:.2f} секунд"
    ...
    def test_bulk_user_update_performance(self, performance_session):
        ...
        # Проверяем результат
        updated_users = performance_session.query(User).filter(
            User.name.like("% Updated")
        ).all()
        assert len(updated_users) == user_count
        
        # Проверяем производительность
        assert update_time < 1.0, f"Обновление {user_count} пользователей заняло {update_time:.2f} секунд"
...
class TestSerializationPerformance:
    ...
    def test_user_serialization_performance(self, performance_session):
        ...
        # Проверяем производительность
        assert serialization_time < 1.0, f"Сериализация {user_count} пользователей заняла {serialization_time:.2f} секунд"
    ...
    def test_complex_user_serialization_performance(self, performance_session):
        ...
        users_with_relations = performance_session.query(User).options(
        ...
        assert serialization_time < 3.0, f"Сериализация {user_count} сложных пользователей заняла {serialization_time:.2f} секунд"
...
class TestQueryOptimizationPerformance:
    ...
    def test_n_plus_1_problem_prevention(self, performance_session):
        ...
        for user in users_bad:
            if user.profile:  # Это вызовет дополнительный запрос для каждого пользователя
        ...
        users_good = performance_session.query(User).options(
            selectinload(User.profile)
        ).all()
        profile_names_good = []
        ...
        assert optimization_ratio > 2.0, f"Оптимизация дала прирост только в {optimization_ratio:.1f}x раз"
    ...
    def test_pagination_performance(self, performance_session):
        ...
        for page in range(0, 5):  # Тестируем первые 5 страниц
            ...
            page_users = performance_session.query(User).order_by(
                User.username
            ...
            assert page_time < 0.1, f"Страница {page} загружалась {page_time:.3f} секунд"
        ...
        assert max_time < avg_time * 2, f"Нестабильная производительность пагинации: avg={avg_time:.3f}, max={max_time:.3f}"
...
class TestMemoryUsagePerformance:
    ...
    def test_large_dataset_memory_efficiency(self, performance_session):
        ...
        assert total_memory_usage < 100, f"Слишком большое использование памяти: {total_memory_usage:.1f} MB"
...
class TestConcurrentPerformance:
    ...
    def test_concurrent_read_performance(self, performance_session):
        ...
        def read_users():
            ...
            try:
                ...
                with Session(concurrent_engine) as session:
                    ...
                    serialized = [UserResponse.model_validate(user) for user in thread_users]
        ...
        assert len(successful_results) == thread_count, f"Не все потоки успешно выполнились: {results}"
