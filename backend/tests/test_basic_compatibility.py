...
class TestBasicModelSchemaCompatibility:
    ...
    def test_user_create_schema_validation(self, user_data):
        ...
        user_create_data["password"] = "SafePassword123!"  # Не содержит email или username
    ...
    def test_user_schema_field_types(self):
        ...
        assert "str" in str(fields["username"].annotation) or fields["username"].annotation == str
        assert "EmailStr" in str(fields["email"].annotation)  # email использует EmailStr
        assert "str" in str(fields["name"].annotation) or fields["name"].annotation == str
