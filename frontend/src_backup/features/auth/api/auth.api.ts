...
export class AuthApi {
  ...
  async confirmPasswordReset(request: PasswordResetConfirm): Promise<{
    ...
    try {
      ...
      throw new Error(
        error.response?.data?.detail || "Password reset confirmation failed"
      );
    }
  }
  ...
  async requestEmailVerification(request: EmailVerificationRequest): Promise<{
    ...
    try {
      ...
      throw new Error(
        error.response.data.detail || "Email verification request failed"
      );
    }
  }
  ...
  async getCurrentUser(): Promise<UserProfile> {
    try {
      ...
      throw new Error(
        error.response.data.detail || "Failed to get current user"
      );
    }
  }
   ...
   */
  async getSocialAuthUrl(provider: 'google' | 'github'): Promise<{ url: string }> {
    try {
      ...
      throw new Error(
        error.response?.data?.detail || `Failed to get ${provider} auth URL`
      );
    }
  }
  ...
  async handleSocialCallback(
    ...
    try {
      const response = await apiClient.post<LoginResponse>(
        `${this.baseUrl}/social/${provider}/callback`,
        { code, state }
      );
      return response.data;
    } catch (error: any) {
      throw new Error(
        error.response?.data?.detail || `${provider} authentication failed`
      );
    }
  }
}
