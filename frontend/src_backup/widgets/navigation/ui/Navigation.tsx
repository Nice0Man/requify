...
import { createNavigationActions } from "@/features/navigation/model/navigationActions";
...
export const Navigation: React.FC<NavigationProps> = ({
  ...
  const navigation = createNavigationActions(navigate, dispatch);
  ...
  const handleItemClick = (item: NavItem) => {
    if (item.children) {
      ...
      navigation.navigateToRoute(item.path);
      if (isMobile && onClose) {
        onClose();
      }
    }
  ...
  const drawerContent = (
    <Stack sx={{ height: "100%" }}>
            ...
            <Stack direction="row" spacing={1} sx={{ mt: 1, mb: 1 }}>
              {quickActions.map((action) => (
                <Tooltip key={action.label} title={action.label}>
                  <IconButton
                    size="small"
                    onClick={() => navigation.navigateToRoute(action.path)}
                    sx={{
                      borderRadius: 2,
                      border: `1px solid ${alpha(theme.palette.divider, 0.1)}`,
                      "&:hover": {
      ...
      >
        ...
        <MenuItem
          onClick={() => {
            navigation.navigateToRoute("/help");
            handleUserMenuClose();
          }}
        >
          <ListItemIcon>
        ...
        <MenuItem
          onClick={() => {
            navigation.navigateToRoute("/feedback");
            handleUserMenuClose();
          }}
        >
          <ListItemIcon>
