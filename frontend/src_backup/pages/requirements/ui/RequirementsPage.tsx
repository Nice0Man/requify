...
interface RequirementSearchParams {
  search?: string;
...
const RequirementsPage: React.FC = () => {
  ...
  // Filters
  const [filters, setFilters] = useState<RequirementSearchParams>({
    search: "",
    projectId: null,
    statusIds: [],
    priorityIds: [],
  ...
  const [selectedBulkStatus, setSelectedBulkStatus] = useState<number | null>(null);
  const [selectedBulkAssignee, setSelectedBulkAssignee] = useState<number | null>(null);
  ...
  // Handlers
  const handleFilterChange = (key: keyof RequirementSearchParams, value: any) => {
    setFilters((prev: RequirementSearchParams) => ({ ...prev, [key]: value }));
  ...
  const handleBulkStatusChange = async () => {
    ...
    try {
      await Promise.all(
        selectedRows.map((id) => 
          requirementsApi.changeRequirementStatus(id, { status_id: selectedBulkStatus })
        )
      );
      setSnackbar({
        open: true,
        message: `Status updated for ${selectedRows.length} requirements`,
        severity: "success",
      });
      setSelectedRows([]);
      loadRequirements();
  ...
  const handleBulkAssign = async () => {
    ...
    try {
      await Promise.all(
        selectedRows.map((id) => 
          requirementsApi.updateRequirement(id, { assigned_to: selectedBulkAssignee })
        )
      );
      setSnackbar({
        open: true,
        message: `Assigned ${selectedRows.length} requirements`,
        severity: "success",
      });
      setSelectedRows([]);
      loadRequirements();
  ...
  return (
    <Box>
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle>Change Status for {selectedRows.length} Requirements</DialogTitle>
        <DialogContent>
          <Box sx={{ mt: 2 }}>
            <Autocomplete
              options={statuses}
              getOptionLabel={(option) => option.name}
              value={statuses.find(s => s.id === selectedBulkStatus) || null}
              onChange={(_, value) => setSelectedBulkStatus(value?.id || null)}
              renderInput={(params) => (
                <TextField
                  {...params}
                  label="New Status"
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle>Assign {selectedRows.length} Requirements</DialogTitle>
        <DialogContent>
          <Box sx={{ mt: 2 }}>
            <Autocomplete
              options={users}
              getOptionLabel={(option) => `${option.first_name} ${option.last_name}` || option.username}
              value={users.find(u => u.id === selectedBulkAssignee) || null}
              onChange={(_, value) => setSelectedBulkAssignee(value?.id || null)}
              renderInput={(params) => (
                <TextField
                  {...params}
                  label="Assign To"
