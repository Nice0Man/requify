...
export const LoginFormWidget: React.FC<LoginFormWidgetProps> = ({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const isValidationError = responseData?.error?.type === "ValidationError" || 
      ...
      if (isValidationError) {
        ...
        if (
          ...
          details.validation_errors.forEach((err: any, index: number) => {
            ...
            if (err.field && err.message) {
              ...
              console.log(`Field error mapped: ${fieldName} -> ${cleanMessage}`);
            }
          });
        ...
        if (errorData?.type === "HTTPException") {
          ...
          console.log("HTTPException message:", message);
          setApiError(message);
        } else {
          // Generic 400 error handling
          const message = errorData?.message || responseData?.message || "Bad request";
          setApiError(message);
        }
      } else if (status === 401) {
        ...
        const details = errorData?.details;
          ...
          console.log("422 Has details?", !!details);
          console.log("422 Has validation_errors?", !!details?.validation_errors);
          
          if (details?.validation_errors && Array.isArray(details.validation_errors)) {
            const fieldErrors: Record<string, string> = {};
            details.validation_errors.forEach((err: any, index: number) => {
              console.log(`Processing 422 login error ${index}:`, err);
              if (err.field && err.message) {
                let fieldName = err.field.replace(/^body\./, '');
                let cleanMessage = err.message.replace(/^Value error,\s*/, '');
                fieldErrors[fieldName] = cleanMessage;
                console.log(`422 Login mapped: ${fieldName} -> ${cleanMessage}`);
              }
            });
            
            if (Object.keys(fieldErrors).length > 0) {
              setErrors(fieldErrors);
              console.log("Set 422 login field errors:", fieldErrors);
            }
            
            const errorMessages = details.validation_errors
              .map((err: any) => err.message?.replace(/^Value error,\s*/, '') || 'Validation error')
              .join(', ');
            setApiError(`Please fix the following: ${errorMessages}`);
          } else {
            const fallbackMessage = errorData?.message || responseData?.message || error?.message || "Validation failed";
            console.log("422 login fallback message:", fallbackMessage);
