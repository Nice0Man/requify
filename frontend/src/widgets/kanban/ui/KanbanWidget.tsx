...
const kanbanTypeConfig = {
  projects: { label: "Projects", icon: <RocketLaunch />, color: "#10b981" },
  requirements: {
    label: "Requirements",
    icon: <Assignment />,
    color: "#3b82f6",
  },
  releases: { label: "Releases", icon: <Rocket />, color: "#8b5cf6" },
  tests: { label: "Tests", icon: <BugReport />, color: "#f59e0b" },
};

const variantConfig = {
  minimal: { label: "Minimal", icon: <ViewList /> },
  compact: { label: "Compact", icon: <ViewModule /> },
  detailed: { label: "Detailed", icon: <ViewComfy /> },
};
...
export const KanbanWidget: React.FC<KanbanWidgetProps> = ({
  ...
  const { availablePriorities, availableAssignees } =
    useMemo(() => {
      const itemsArray = Array.isArray(items) ? items : [];
      const priorities = Array.from(
        new Set(itemsArray.map((item: KanbanCard) => item.priority).filter(Boolean))
      ) as KanbanItemPriority[];

      const assignees = Array.from(
        new Set(itemsArray.map((item: KanbanCard) => item.assignee).filter(Boolean))
      );
  ...
  const handleTypeChange = useCallback(
    (_event: React.MouseEvent<HTMLElement>, newType: KanbanType | null) => {
      if (newType && newType !== currentType) {
        ...
        setNotification({
          open: true,
          message: `Switched to ${kanbanTypeConfig[newType].label}`,
          severity: "info",
        });
      }
    },
  ...
  const handleVariantChange = useCallback(
    (
      ...
      if (newVariant && newVariant !== currentVariant) {
        ...
        setNotification({
          open: true,
          message: `Switched to ${variantConfig[newVariant].label} view`,
          severity: "info",
        });
      }
    },
        ...
        >
          <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
            ...
            <Typography variant="h6" fontWeight={600}>
              {kanbanTypeConfig[currentType].label} Board
            </Typography>
          </Box>
