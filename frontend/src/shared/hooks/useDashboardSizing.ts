...
export const useDashboardSizing = (config: DashboardSizingConfig): DashboardSizes => {
  ...
  const sizes = useMemo(() => {
    ...
    const totalMultiplier = densityMultiplier * layoutMultiplier * screenMultiplier * masonryMultiplier;
    
    // Рассчитываем основные размеры
    const widgetHeight = Math.round(baseSizeConfig.widgetHeight * totalMultiplier);
    ...
    const headerHeight = Math.round(baseSizeConfig.headerHeight * densityMultiplier);
    const chartHeight = Math.round(baseSizeConfig.chartHeight * totalMultiplier);
    ...
    const gridConfig = {
      ...
      columns: (() => {
        ...
        if (isUltrawide) return mode === "minimal" ? 2 : mode === "compact" ? 3 : 3;
        if (isWidescreen) return mode === "minimal" ? 2 : mode === "compact" ? 2 : 3;
    ...
    // Typography размеры
    const typography = {
      title: mode === "minimal" ? "1.25rem" : mode === "compact" ? "1.375rem" : "1.5rem",
      subtitle: mode === "minimal" ? "0.875rem" : mode === "compact" ? "0.9375rem" : "1rem",
      body: mode === "minimal" ? "0.8125rem" : mode === "compact" ? "0.875rem" : "0.9375rem",
      caption: mode === "minimal" ? "0.75rem" : mode === "compact" ? "0.8125rem" : "0.875rem",
    };
    
    // Elevation для теней
    const elevation = {
      card: mode === "fullscreen" 
        ? `0 1px 8px ${theme.palette.action.hover}10`
        : `0 2px 20px ${theme.palette.action.hover}20`,
      widget: mode === "fullscreen"
