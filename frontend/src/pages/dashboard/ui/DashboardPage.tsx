...
import { useAuth, usePermissions } from "@/features/auth/model/auth.context";
...
const DashboardPage: React.FC = () => {
  ...
  const loadDashboardData = async (isRefresh = false) => {
    try {
      ...
      // Prepare parallel requests using new API endpoints
      const requests = [
        dashboardApi.getDashboardStats(),
        dashboardApi.getMyDashboard(),
        dashboardApi.getMyProjects(),
        dashboardApi.getMyRequirements(),
        dashboardApi.getMyNotifications(),
      ...
      const dashboardStats = statsResponse as DashboardStats;
      const myDashboardData = myDashboardResponse as DashboardData;
      const myProjects = myProjectsResponse as QuickProject[];
      const myRequirements = myRequirementsResponse as QuickRequirement[];
      const myNotifications = myNotificationsResponse as DashboardNotification[];
      const recentActivity = recentActivityResponse as ActivityItem[];

      // Combine main dashboard data with fallbacks
      const combinedData: DashboardStats = {
        ...
        project_performance: dashboardStats.project_performance || [],
        trending_metrics: dashboardStats.trending_metrics || [],
        ...
        requirements: dashboardStats.requirements || {
          total: 0,
          approved: 0,
          pending: 0,
          in_review: 0,
        },
        releases: dashboardStats.releases || {
          total: 0,
          planned: 0,
          in_progress: 0,
          deployed: 0,
        },
        team: dashboardStats.team || {
          total_users: 0,
          active_users: 0,
          online_users: 0,
        },
      ...
      if (isAdmin && adminResponses.length >= 2) {
        ...
        if (systemInfoResponse.success && healthResponse.success) {
          const adminStats: AdminPanelData = {
            ...
            security_alerts: systemInfoResponse.data?.security_alerts || 0, // Would come from security endpoint
            active_users_today:
              systemInfoResponse.data?.api_health?.active_sessions || 0,
            failed_logins_today: systemInfoResponse.data?.security_logs?.failed_logins || 0, // Would come from security logs
            backup_status: {
              last_backup:
