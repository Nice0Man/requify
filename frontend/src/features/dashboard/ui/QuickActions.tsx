...
export const QuickActions: React.FC = () => {
  ...
  const ActionCard: React.FC<{ action: typeof quickActions[0]; index: number }> = ({ action, index }) => {
    return (
        ...
        <Card 
          sx={{ 
            ...
            '&:hover': {
              ...
              boxShadow: `0 20px 40px ${alpha(theme.palette[action.color as keyof typeof theme.palette].main, 0.2)}`,
            },
            border: `1px solid ${alpha(theme.palette[action.color as keyof typeof theme.palette].main, 0.1)}`,
            background: `linear-gradient(135deg, ${alpha(theme.palette[action.color as keyof typeof theme.palette].main, 0.05)} 0%, transparent 50%)`,
          }}
  ...
  return (
    ...
    >
      <Box sx={{ mb: 3 }}>
        ...
        <Typography variant="body2" color="text.secondary">
          Создавайте новые элементы одним кликом
        </Typography>
      </Box>
