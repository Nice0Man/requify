...
export const InfiniteCarousel: React.FC<InfiniteCarouselProps> = React.memo(({
  ...
  const handleDragStart = React.useCallback((event: any, info: PanInfo) => {
  ...
  const handleDrag = React.useCallback((event: any, info: PanInfo) => {
  ...
  }, [navigateCarousel, carousel.goToSlide, carousel.isTransitioning, items.length]);
  ...
  }, [handleWheel, handleKeyDown, handleTouchStart, handleTouchEnd, carousel.containerRef]);
  ...
  const gradientBase = theme.palette.mode === 'dark' 
    ...
    >
      ...
      <MotionBox
        ...
        transition={{
          ...
          duration: dragState.isDragging ? 0.2 : carousel.isTransitioning ? 0.6 : 0.8,
          ease: dragState.isDragging ? [0.25, 0.46, 0.45, 0.94] : [0.16, 1, 0.3, 1],
      ...
      >
        {carousel.extendedItems.map((item, index) => {
          ...
          const maxRenderDistance = Math.ceil(carousel.carouselParams.visibleCards) + 2;
            ...
            return (
              <Box
                ...
                sx={{
                  ...
                  marginRight: index < carousel.extendedItems.length - 1 
      ...
      {autoPlay && carousel.isAutoPlay && !dragState.isDragging && !carousel.isHovering && (
        <MotionBox
